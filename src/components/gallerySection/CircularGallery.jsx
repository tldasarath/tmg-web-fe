...
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
    autoBind(this); // ✅ ensures all methods use correct `this`
    ...
    this.startAutoScroll(); // ✅ auto scroll added
  }
  createRenderer() {
  ...
  startAutoScroll() {
    this.autoScrollInterval = setInterval(() => {
      this.scroll.target += this.scrollSpeed * 2; // scroll right every 2 seconds
    }, 2000);
  }
  onTouchDown(e) {
    this.isDown = true;
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 }) {
  ...
  return <div className="circular-gallery w-full h-full" ref={containerRef}></div>;
}
